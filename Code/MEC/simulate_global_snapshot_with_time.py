...
def simulate_global_coverage(all_eds: List[ED], es_ref: EdgeServer,
                              dt: float = 0.5,
                              max_time: float = 10.0) -> Dict[float, List[int]]:
    """

    step1. saare EDs ko ek single global clock ke sath simulate karo,
                    dt (default 0.5 sec) ke gap se, max_time tak

    step2. return karega ek hi dict -> { timestamp -> [uss time coverage
           ke andar jo EDs hai unke stable_id] }
    ...
    IMPORTANT -> ED.id use mat karna identity ke liye, vo galat hai:

        @property
        def id(self) -> int: ED.ed_counter += 1; return ED.ed_counter

    yea koi stored per-device id nhi hai. yea ek property hai jo har baar
    .id read hote hi ek SHARED CLASS-LEVEL counter ko +1 kar deti hai aur
    jo bhi abhi counter ki value hai vo return kar deti hai. same object
    ka .id do baar read karo toh do alag number aayenge, aur vo counter
    saare ED instances ke beech shared hai (poore process mea ek hi hai)

    isi liye pehle jo bhi "ED IDs: [...]" print hota tha usme kabhi bhi
    ek hi ED do alag timestamps pe repeat nhi hota tha - yea real result
    nhi tha device movement ka, bas property ka side-effect tha jisse
    aisa lagna hi tha

    (agar Algos/Classes/ED.py mea id fix kar diya hai - ed_counter ko
    __post_init__ mea ek hi baar assign karke - toh ab ed.id bhi trust
    kar sakte ho, but yaha stable_id (list ka fixed index) use kar rha
    hu taki ED.py fix ho ya na ho, yea function hamesha sahi chale)
    ...
    # step6. coordinates chahiye ho toh yaha se milenge (id -> list wale
    # return type ko chhede bina, extra attribute ki tarah attach kar diya)
    simulate_global_coverage.last_states = global_snapshots_states

    return global_snapshots


def print_snapshots(snapshots: Dict[float, List[int]]) -> None:
    # bas print karne ka kaam, koi naya logic nhi
    for t in sorted(snapshots):
        print(f"\n========== Time = {t:.1f} sec ==========")
        print(f"Number of EDs inside = {len(snapshots[t])}")
        print("ED stable_ids:", snapshots[t])
...
def run(max_time: float = 10.0, dt: float = 0.5):
    ...
    # step3. ab single global clock wali simulation chalao
    snapshots = simulate_global_coverage(eds, es_copy, dt=dt, max_time=max_time)

    # step4. print kar do result
