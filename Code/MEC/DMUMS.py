...
def replace_model(ed: ED, eds: List[ED], es:EdgeServer): #logic to eliminate model
    ...
    while(ed.model.storage > (server_storage_left+freed_storage) and temp != set()): #storage constraints to accomodate new model
        ...
        if(util < new_model_util): #victim model ka utility new model utility se kam hain
            ...
            freed_storage += victim_model.storage #add to left storage
    ...
    enough_space = (server_storage_left + freed_storage) >= ed.model.storage
    higher_utility = new_model_util > total_util(victim_models, es, eds) if victim_models else False

    if enough_space and higher_utility:
        # print(f"Models in {victim_models} to be replaced with demanded model")
